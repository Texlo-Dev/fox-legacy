...
export default {
  ...
  async asyncData({ app, route, params: { guildid } }) {
    ...
    const [commands, config, channels, roles, leveling] = await Promise.all([
      ...
      (await app.$axios.get(`/api/guilds/${guildid}/leveling`, {
        headers: { Authorization: secret.encrypt(app.$auth.user.id) }
      })).data
    ]);
    return {
  ...
  methods: {
    async validatePromo() {
      ...
      result
        ? this.addPromoRole(this.roleData.role, this.roleData.rank)
        : alert("Incorrect Parameters.");
    },
    dropdownSave(key, meta, item, bool) {
      return this.levelingUpdate(key, item, { meta, bool }).then(
        (this.prompts = [])
      );
    },
    async confirmDelete(role) {
      this.$dialog.confirm({
        ...
        message: `Are you sure that you'd like to remove the role "${
          role.name
        }"?`,
        cancelText: "Cancel",
        confirmText: "Delete",
        ...
        onConfirm: async () => {
          try {
            this.leveling.promoRoles.splice(
              this.leveling.promoRoles.indexOf(role, 1)
            );
    ...
    async levelingUpdate(key, value, options) {
      try {
        this.leveling = await API.levelingUpdate(
          key,
          value,
