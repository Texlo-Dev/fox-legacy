...
export default {
  ...
  methods: {
    async validateForm() {
      ...
      result
        ? this.toggleRole(this.roleData)
        : this.$toast.open("Incorrect parameters.");
    },
    async deleteRole(role) {
      this.$dialog.confirm({
        title: "Delete Reaction Role",
        message: `Are you sure that you'd like to delete the reaction role "${
          role.name
        }"?`,
        cancelText: "Cancel",
        confirmText: "Delete",
        type: "is-danger",
        onConfirm: async () => {
          this.config.reactionRoles.splice(
            this.config.reactionRoles.indexOf(role),
            1
          );
