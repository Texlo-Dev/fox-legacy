  ...
  <section class="section">
    <div class="container">
      <nav class="level is-mobile">
        ...
        <div class="level-left">
          <a class="button is-danger" @click="confirmPkg('Moderation')"
    ...
    <div v-if="!loading && config" class="container">
      ...
      <div class="columns is-multiline">
                ...
                <span v-if="config.modLogging">
                  ...
                  <section>
                    ...
                    <div v-if="!prompts.includes('modlog')">
                      <a
                        v-if="config.modlogChannel"
                        id="channel"
                        :href="
                          `https://discordapp.com/channels/${
                            $route.params.guildid
                          }/${config.modlogChannel.id}`
                        "
                      >#{{ config.modlogChannel.name }}
                      </a>
                      <p v-else class="subtitle has-text-white">No Channel</p>
                      &nbsp;<button
                        class="button is-small is-primary"
                        @click="prompts.push('modlog')"
                      >
                        Change
                      </button>
                    </div>
            ...
            <div class="content">
              <h3 class="has-text-white has-text-left">
                ...
                <div v-if="!prompts.includes('mute')">
                  <a v-if="config.muteRole" id="channel"
                  >@{{ config.muteRole.name }}
                  </a>
                  <p v-else class="subtitle has-text-white">No Role</p>
                  &nbsp;<button
                    class="button is-small is-primary"
                    @click="prompts.push('mute')"
                  >
                    Change
                  </button>
                </div>
              ...
              <p>
                Note: Mr.Fox will attempt to update the chat permissions for the
                specified role.
              </p>
            </div>
          </div>
        ...
        <div class="column is-half">
          ...
          <div class="box">
            <div class="content">
              <h3 class="has-text-white has-text-left">Settings</h3>
              <div class="field">
                <b-checkbox
                  v-model="config.delModCmds"
                  type="is-grey"
                  @click.native="
                    settingUpdate('delModCmds', !config.delModCmds)
                  "
                >
              ...
              <div class="field">
                <b-checkbox
                  v-model="config.msgAfterMod"
                  type="is-grey"
                  @click.native="
                    settingUpdate('msgAfterMod', !config.msgAfterMod)
                  "
                >
        ...
        <div class="column is-half">
          <div class="box">
            <div class="content">
              <h3 class="has-text-white has-text-left">Warning Points</h3>
              <p class="subtitle has-text-white">Automatic kick threshold</p>
              <b-field>
                <b-input
                  id="grey"
                  v-model="config.kickPoints"
                  type="number"
                  min="10"
                  max="100000"
                />
                <button
                  class="button is-primary"
                  @click="settingUpdate('kickPoints', config.kickPoints)"
                >
                  Update
                </button>
              </b-field>
              ...
              <b-field>
                ...
                <button
                  class="button is-primary"
                  @click="settingUpdate('banPoints', config.banPoints)"
                >
                  Update
                </button>
              </b-field>
    ...
    <b-modal :active.sync="modalActive" size="is-large" has-modal-card>
      <div class="modal-card">
        ...
        <section class="modal-card-body">
          <form id="serverlog" @submit.prevent="logValidate">
            ...
            >
              <b-select
                id="modalselect"
                v-model="config.serverlogChannel"
                name="channel"
              >
                <option
                  v-for="channel of channels"
            ...
            <b-field label="Event List" custom-class="has-text-white">
              <section
                id="checkbox"
                class="has-text-centered has-background-black"
              >
                <div class="box">
                  <div class="content">
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="channelCreate"
                    >
                      <p>Channel Created</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="channelDelete"
                    >
                      <p>Channel Deleted</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="channelUpdate"
                    >
                      <p>Channel Updated</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="emojiCreate"
                    >
                      <p>Emoji Created</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="emojiDelete"
                    >
                      <p>Emoji Deleted</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="roleCreate"
                    >
                      <p>Role Created</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="roleDelete"
                    >
                      <p>Role Deleted</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="roleUpdate"
                    >
                      <p>Role Updated</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="guildMemberAdd"
                    >
                      <p>Member Join</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="guildMemberRemove"
                    >
                      <p>Member Leave</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="guildMemberUpdate"
                    >
                      <p>Member Detail Change</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="messageDelete"
                    >
                      <p>Message Deleted</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="messageUpdate"
                    >
                      <p>Message Updated</p>
                    </b-checkbox>
                    <b-checkbox
                      v-model="config.enabledEvents"
                      native-value="voiceStateUpdate"
                    >
        ...
        <footer class="modal-card-foot">
          ...
          <button class="button is-primary" type="submit" form="serverlog">
            Save
          </button>
        </footer>
      </div>
