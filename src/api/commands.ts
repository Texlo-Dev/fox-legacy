...
router.patch("/:guildID", authMiddleware, async (req, res) => {
    ...
    if (!userID || !command || !guildID || bool == null) return res.status(400).json({ message: "Missing Parameters" });
    try {
...
router.get("/:pkg", authMiddleware, async (req, res) => {
    ...
    commands.forEach(command => {
        if ((settings.disabledCommands ? settings.disabledCommands : []).indexOf(command.name) > -1) command.enabled = false;
