...
export default class Banking {
  ...
  public static checkEligibility(
    ...
    const excludedRoles: Role[] = (member.guild as FoxGuild).banking
      .excludedRoles;
    const excludedChannels: TextChannel[] = (member.guild as FoxGuild).banking
      .excludedChannels;
    if (
  ...
  public static async giveDaily(
    ...
    if (entry.get("timeUntilDaily") >= Date.now()) {
      return time;
    }
  ...
  public async listen(message: FoxMessage): Promise<Banking> {
    ...
    if (!Banking.validate(message)) {
      return;
    }
    if (message.command) {
      return;
    }
    if (!message.guild.packages.get("Economy").enabled) {
      return;
    }
    const isEligible: boolean = Banking.checkEligibility(
      message.member,
      message.channel
    );
    if (!isEligible) {
