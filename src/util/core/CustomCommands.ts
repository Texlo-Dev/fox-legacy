...
export default class CustomCommands extends Collection<any, any> {
        ...
        this.actions = {
            async addRole(message: FoxMessage, role: any): Promise<void> {
                if (role.match(/^(\\d{17,19})$/)) {
                    ...
                    await message.member.roles.add(message.guild.roles.find((r) => r.name === role)); }
            },
            async removeRole(message: FoxMessage, role: any): Promise<void> {
                if (role.match(/^(\\d{17,19})$/)) {
                    message.member.roles.remove(role);
                } else if (message.guild.roles.some((r) => r.name === role)) {
                    await message.member.roles.remove(message.guild.roles.find((r) => r.name === role));
                }
            },
    ...
    public async reloadAll(): Promise<boolean> {
        const commands: CCMongo[] = await this.guild.client.mongo.customcommands.find({ guildID: this.guild.id });
        if (!commands) { return; }
    ...
    public async remove(command: string): Promise<Object> {
        ...
        const entry: CCMongo = await this.guild.client.mongo.customcommands
            .findOne({ guildID: this.guild.id, name: command });
