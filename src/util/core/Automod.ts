...
export const invProtect: (message: FoxMessage) => void = (message: FoxMessage) => {
    if (!message.guild || !message.member) { return; }
    const sp: boolean = message.guild.config.invProtected;
    if (sp && message.guild.me.hasPermission("MANAGE_MESSAGES") && !message.guild.perms.check("automod.sendinvites", message) && message.content.match(inviteRegex)) { // tslint:disable-line
        message.delete()
            .catch(() => 0);
        message.author.send(
...
export const massProtect: (message: FoxMessage) => void = async (message: FoxMessage) => {
...
export const spamProtect: (message: FoxMessage) => void = (message: FoxMessage) => {
    if (!message.guild) { return; }
    const sp: ServerProtect = message.guild.serverprotect;
