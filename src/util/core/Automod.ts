import { FoxMessage, ServerProtect } from "..";

const inviteRegex = new RegExp(/\b(?:https?:\/\/)?(?:www\.)?(?:discordapp\.com\/invite\/|discord\.gg\/)([\w-]{2,32})/gi);
export const badWords = (message: FoxMessage) => {
    if (!message.guild || !message.guild.config.bwProtected || message.guild.perms.check("automod.freespeech", message)) return;
    if (message.guild.config.allowedBwChannels) for (const channel of message.guild.config.allowedBwChannels) if (message.channel.id === channel.id) return;
...
export const invProtect = (message: FoxMessage) => {
    if (!message.guild || !message.member) return;
    const sp: string[] = message.guild.config.invProtected;
...
export const massProtect = async (message: FoxMessage) => {
    if (!message.guild) return;
    const sp: ServerProtect = message.guild.serverprotect;
    const spEnabled: string[] = message.guild.config.massProtected;
...
export const spamProtect = (message: FoxMessage) => {
    if (!message.guild) return;
    const sp: ServerProtect = message.guild.serverprotect;
    ...
    const struct: object = { member: message.member, messages: 0, time: Math.floor(Date.now()) };
