...
export const checkUnmute: (client: FoxClient) => Promise<boolean> = async (
  client
): Promise<boolean> => {
...
export const checkIfMute: (member: GuildMember) => Promise<boolean> = async (
  member: GuildMember
): Promise<boolean> => {
  ...
  if (check) {
    ...
    if (muteRole) {
      member.roles.add(muteRole).catch(() => 0);
    }
  } else {
    return false;
  }
