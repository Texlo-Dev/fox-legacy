...
class ServerProtect {
  public static massRole(str: string): RegExpMatchArray {
    ...
    return str.match(roleregex)
      ? str.match(roleregex)
      : str.match(everyoneregex);
  }
  ...
  public async ban(member: GuildMember, reason: string): Promise<void> {
    await member
      .send(
        `Mention spam has been detected in your message, and you have been banned, as part of my spam protection package. If you feel like this was a mistake, DM ${
          member.guild.owner.user.tag
        }.`
      )
      .catch(() => 0); // tslint:disable-line
  ...
  public async warnMute(member: GuildMember, reason: string): Promise<void> {
    ...
    member
      .send(
        "I have detected a great amount of spam in your message(s), and I have muted you, as part of my spam protection package."
      )
      .catch(() => 0); // tslint:disable-line
