...
export default Structures.extend("Message", mes => {
  class FoxMessage extends mes {
    public static combineContentOptions(content: any, options: any): any {
      if (!options) {
        return this._isObject(content) ? content : { content };
      }
    ...
    public async error(
      ...
      content =
        this.guild.config.language !== "English" // tslint:disable-line
    ...
    public async sendMessage(
      ...
      try {
        if (options.embed && this.guild.config.language !== "English") {
          ...
          if (options.embed.description) {
            options.embed.description = options.embed.description.replace(
              /xyz/gi,
              "\n"
            );
          }
          ...
          for (const field of options.embed.fields) {
            ({ text: field.name } = await this.client.translate(field.name, {
              from: "en",
              to: this.client.locales[this.guild.config.language]
            }));
          }
        ...
        return this.channel.send(
          `<:nicexmark:495362785010647041> I could not translate this command. ${
            error.message
          }`
        );
      }
      ...
      if (this.responses && typeof options.files === "undefined") {
        ...
        if (Array.isArray(content)) {
          ...
          return Promise.all(promises).then(prm => {
            this.responses = prm;

            return prm;
          });
        } else if (Array.isArray(this.responses)) {
          for (let i: number = this.responses.length - 1; i > 0; i--) {
            this.responses[i].delete();
          }
      ...
      return this.channel.send(content, options).then(mess => {
    ...
    public async sendPrompt(
      ...
      const collected: any = await this.channel
        .awaitMessages(filter ? filter : m => m.author.id === this.author.id, {
          max: 1,
          time,
          errors: ["time"]
        })
      ...
      if (collected.first().content.match(/cancel/i)) {
        return 0;
      } else {
