...
export default class Queue implements QueueInfo {
    ...
    public endAllSongs(): boolean | Promise<Message | Message[]> {
        ...
        if (!serverQueue.songs.length) {
            return this.textChannel.send("<:nicexmark:495362785010647041>  There is nothing playing for me to stop.");
        }
        serverQueue.songs = [];
        if (!serverQueue.connection) { return undefined; }
    ...
    public async skip(member: GuildMember): Promise<Message | Message[]> {
        ...
        if (!serverQueue) {
            return this.textChannel.send("<:nicexmark:495362785010647041> Sorry, but there was nothing playing for me to skip."); // tslint:disable-line
        }
        if (serverQueue.skippers.includes(member.id)) {
            return this.textChannel.send("<:nicexmark:495362785010647041> Sorry, but you have already voted to skip!");
        }
        ...
        if (serverQueue.skippers.length > Math.floor(member.voice.channel.members.size - 1) / 3) {
            ...
            return this.textChannel.send(`<:check:314349398811475968> Your skip has been added! To complete the skip, you need ${Math.ceil((member.voice.channel.members.size - 1) / 2) - serverQueue.skippers.length} more votes.`); // tslint:disable-line
        }
    }
