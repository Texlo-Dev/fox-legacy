...
class FoxMusic {
    ...
    async getID(args, message, member) {
        if (args.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
            ...
            try {
                ...
                try {
                    ...
                    message.send({ embed });
                    try {
                        var res = await message.channel.awaitMessages(m => m.author.id === message.author.id, {
                            max: 1,
                            time: 15000,
                            errors: ['time']
                        });
    ...
    async handleVideo(video, message, member, voiceChannel, playlist = false) {
