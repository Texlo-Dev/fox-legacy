...
export default class Giveaway implements GiveawayInfo {
  ...
  public async end(): Promise<Object> {
    if (!this.running) {
      throw new Error("Giveaway must be running in order to end.");
    }
    this.ended = true;

    try {
      const guild: FoxGuild = this.guild.client.guilds.get(
        this.guildID
      ) as FoxGuild;
  ...
  public async pause(): Promise<Object> {
    if (this.paused || !this.running) {
      throw new Error(
        "Giveaway must not either be already paused and currently running."
      );
    }
    try {
      const guild: FoxGuild = this.guild.client.guilds.get(
  ...
  public async reroll(): Promise<Object> {
    if (this.running) {
      throw new Error("Giveaway must not be running to reroll.");
    }
    try {
      const guild: FoxGuild = this.guild.client.guilds.get(
        this.guildID
