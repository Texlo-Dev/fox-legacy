...
export default class GiveawayStore extends Collection<any, any> {
  ...
  public async add(name: string, giveaway: any): Promise<object> {
    if (super.has(name)) {
      throw new Error("Giveaway Already added.");
    }
    const data: any = {
      name,
      guildID: this.guild.id,
    ...
    try {
      ...
      const embed: MessageEmbed = new MessageEmbed()
        .setAuthor(
          "There's a new GIVEAWAY in town!",
          this.client.user.displayAvatarURL()
        )
        .setColor(0xf37934)
        .setTitle(`Giveaway prize: ${gw.name}`)
        .setDescription(
          `In order to enter this giveaway, make sure to react with ${
            giveaway.reactionEmote
              ? this.guild.emojis.get(giveaway.reactionEmote.id)
              : "<:phat:495031665803001876>"
          }.\nPossible winners: **${gw.maxWinners}**\nYou have **${
            gw.timeRemaining
          }** remaining to enter.`.replace(
            gw.timeRemaining.toString(),
            `${duration(gw.timeRemaining, "milliseconds").format(
      ...
      const m: any = await (this.guild.channels.get(
        giveaway.channel.id
      ...
      await entry.save();
      gw.reactionEmote
        ? m.react(giveaway.reactionEmote.id)
        : m.react("495031665803001876");
  ...
  public async listenGiveaway(giveaway: Giveaway): Promise<Object> {
    ...
    if (
      ...
      if (!reaction || !reaction.users.filter(u => !u.bot).size) {
        ...
        embed.description =
          "No users reacted, so the giveaway could not proceed. Please have people react, then reroll."; // tslint:disable-line
        // @ts-ignore
        ...
        reaction.users.filter(u => !u.bot).size < giveaway.maxWinners
      ) {
        embed.title = `Giveaway Name: ${giveaway.name}`;
        embed.description =
          "Not enough users reacted, so the giveaway was cancelled.";
        // @ts-ignore
        embed.timestamp = new Date(giveaway.endDate);
        embed.footer.text = "The Giveaway Ended At";
        message.edit({ embed });
        ...
        const winner: any[] = reaction.users
          .filter(u => !u.bot)
        ...
        embed.timestamp = new Date(giveaway.endDate);
        embed.footer.text = "The Giveaway Ended At";
        embed.author.name = `We have ${
          winner.length > 1 ? "some WINNERS!" : "a WINNER!"
        }`;
        channel.send(
          `Congratulations to ${
            winner.length > 1 ? winner.join(", ") : winner[0]
      ...
      embed.description = embed.description.replace(
        `${duration(gw.get("timeRemaining"), "milliseconds").format(
          "d [days], h [hours], m [minutes and] s [seconds]"
        )}`,
        `${duration(newTime, "milliseconds").format(
          "d [days], h [hours], m [minutes and] s [seconds]"
        )}`
      ); // tslint:disable-line
  ...
  public async pause(giveaway: Giveaway): Promise<Object> {
    ...
    if (channel && message && !giveaway.paused) {
      const embed: MessageEmbed = message.embeds[0];
      embed.description =
        "This giveaway is currently paused. You can still react, but the timer will start when the giveaway is resumed."; // tslint:disable-line
  ...
  public async reroll(giveaway: Giveaway): Promise<Object> {
    ...
    if (channel && message) {
      ...
      let reaction: any = giveaway.reactionEmote
        ...
        : message.reactions.get("495031665803001876");
      if (reaction) {
        reaction = await reaction.users.fetch();
      }
      if (!reaction || !reaction.filter(u => !u.bot).size) {
        embed.title = `Giveaway Name: ${giveaway.name}`;
        embed.description =
          "Reroll failed due to no new reactions. Ensure that users react, and reroll again.";
        message.edit({ embed });
        ...
        embed.title = `Giveaway Name: ${giveaway.name}`;
        embed.description =
          "Not enough users reacted, so the reroll was cancelled.";
        message.edit({ embed });

        return super.array();
      } else {
        const winner: User[] = reaction
          .filter(u => !u.bot)
          .random(giveaway.maxWinners);
        embed.title = `Giveaway Name: ${giveaway.name}`;
        embed.description = `${
          winner.length > 1
            ? `New Winners: ${winner.join(", ")}`
            : `New Winner: ${winner[0]}`
        ...
        channel.send(
          `${
            winner.length > 1 ? winner.join(", ") : winner[0]
          }, you are the new ${winner.length > 1 ? "winners!" : "winner!"}`
        ); // tslint:disable-line
        message.edit({ embed });
    ...
    if (channel && message && giveaway.paused) {
      ...
      embed.description = embed.description.replace(
        ...
        `In order to enter this giveaway, make sure to react with ${
          ...
          giveaway.maxWinners
        }**\nYou have **${newEnd}** remaining to enter.`.replace(
          newEnd.toString(),
          `${duration(newEnd, "milliseconds").format(
            "d [days], h [hours], m [minutes and] s [seconds]"
          )}`
        ) // tslint:disable-line
      );
      ...
      gw.set({
        ...
        endDate: gw.get("endDate") + giveaway.paused,
        timeRemaining: newEnd
      });
      await gw.save();
      await this._cache();
