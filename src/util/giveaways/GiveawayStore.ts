...
export default class GiveawayStore extends Collection<any, any> {
    ...
    public async _cache(): Promise<boolean> {
        ...
        for (const giveaway of mapped) {
            const gw = new Giveaway(this.guild, giveaway);
    ...
    public array(): Giveaway[] {
        ...
        for (const gw of arr) {
            ...
            // @ts-ignore
            gw.timeRemaining = duration(gw.timeRemaining, "milliseconds").format(`d [days], h [hours], m [minutes], s [seconds]`);
        }
        return arr;
    }
    ...
    public async add(name: string, giveaway: any): Promise<object> {
        ...
        const newGiveaway = new this.client.mongo.giveaways(data);
        try {
            await newGiveaway.save();
            const gw = new Giveaway(this.guild, data);
            ...
            gw.messageID = m.id;
            super.set(gw.name, gw);
            const entry: Giveaways = await this.client.mongo.giveaways.findOne({ guildID: this.guild.id, endDate: gw.endDate, name: gw.name });
