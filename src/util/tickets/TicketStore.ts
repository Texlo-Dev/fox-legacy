...
export default class TicketStore extends Collection<any, any> {
  ...
  public async add(options: any, member: FoxUser): Promise<Object> {
    if (!this.logchannel)
      throw new Error(
        "No Log Channel has been set, please set one before continuing."
      );
    if (!this.category)
      throw new Error(
    ...
    try {
      const count: number = await Tickets.count({
        guildID: this.guild.id,
        type: "tkt"
      });
      ...
      const mentionroles: string[] = this.guild.roles
        .filter(r => this.guild.perms.check("ticket.agent", r))
        .map(r => r.toString());
      const logchannel: TextChannel = this.guild.channels.get(
        this.logchannel.id
      ) as TextChannel;
      const logembed: MessageEmbed = new MessageEmbed()
        .setColor(this.client.brandColor)
        .setAuthor(
          `Ticket ${ticket.id} | ${member.tag}`,
          member.displayAvatarURL()
        )
        .setDescription(
          `New Support ticket from ${member.username}.
            **Topic:** ${ticket.topic}
            To accept this ticket, simply run \`${
              this.guild.config.prefix
            }tclaim ${ticket.id}\`.
            `
        )
        .setTimestamp()
        .setFooter(
          `${this.client.user.username} Tickets`,
          this.client.user.displayAvatarURL()
        );
      await logchannel.send(this.pingagents ? mentionroles.join(", ") : null, {
        embed: logembed
      });
      ...
      const tktchannel:
        | TextChannel
        | VoiceChannel = (await this.guild.channels.create(
        `ticket-${ticket.id}`,
        {
          type: "text",
          topic: ticket.topic,
      ...
      const recent: Tickets = await Tickets.findOne({
        guildID: this.guild.id,
        type: "tkt",
        id: ticket.id
      });
  ...
  private async _settings(): Promise<TicketStore> {
    ...
    if (!settings) {
      ...
      return ent.save().then(() => this._settings());
    }
