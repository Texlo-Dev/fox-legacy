...
export default class PollStore extends Collection<any, any> {
        ...
        try {
            ...
            if (pl.type === "simple") {
                ...
                for (const reaction of obj.possibleAnswers) {
                    const emote: string = `${numbers[String(obj.possibleAnswers.indexOf(reaction) + 1)]}`;
                    await m.react(emote);
                }
            }
            pl.messageID = m.id;
            super.set(pl.name, pl);
            const entry: Polls = await this.guild.client.mongo.polls.findOne({ guildID: this.guild.id, name: pl.name });
    ...
    public async gatherData(poll: Poll): Promise<object> {
        ...
        const channel: TextChannel = this.guild.channels.get(poll.channel.id) as TextChannel;
        ...
        try {
            const pl: Polls = await this.guild.client.mongo.polls.findOne({ guildID: this.guild.id, name: poll.name });
            ...
            for (const answer of embed.fields) {
                ...
                const reacted: number = message.reactions.find(m => answer.name.indexOf(m.emoji.name) > -1).count;
                answer.value = `Members Voted: ${reacted}`;
                responses
                    .find(r => r.name.indexOf(poll.type === "open" ? answer.name : answer.name.split(" ")[1]) > -1)
                    .count = reacted;
                if (poll.type === "open") {
                    responses.find(r => r.name === answer.name).name = answer.name.substring(3);
                }
            }
