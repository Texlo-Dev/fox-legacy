import { GuildMember, MessageMentions, TextChannel } from "discord.js";
...
export default class Leveling {
  public static generateXp(level: number): number {
    ...
    if (level >= 35) {
      ...
      return Leveling.randomNum(110, 115);
    } else {
      return Leveling.randomNum(10, 15);
    }
  }
  ...
  public static checkEligibility(
    ...
    const excludedRoles: Role[] = (member.guild as FoxGuild).leveling
      .excludedRoles;
    const excludedChannels: TextChannel[] = (member.guild as FoxGuild).leveling
      .excludedChannels;
    if (
      excludedRoles.length &&
      excludedRoles.some(role => member.roles.has(role.id))
    ) {
      return undefined;
    } else if (
      excludedChannels.length &&
      excludedChannels.some(c => c.id === channel.id)
    ) {
      return undefined;
    }
  ...
  public async _loadSettings(): Promise<void> {
    ...
    if (!settings) {
      const levelsettings: FoxLeveling = new this.guild.client.mongo.leveling({
        guildID: this.guild.id,
        type: "settings"
      });
  ...
  public async listen(message: FoxMessage): Promise<void> {
    ...
    if (message.content.startsWith(message.guild.config.prefix)) {
      return;
    }
    if (!message.guild.packages.get("Leveling").enabled) {
      return;
    }
    if (!Leveling.validate(message)) {
      return;
    }
    const isEligible: boolean = Leveling.checkEligibility(
      message.member,
      message.channel
    );
    if (!isEligible) {
    ...
    const memberdata: FoxLeveling = await this.guild.client.mongo.leveling.findOne(
      {
    ...
    if (memberdata) {
      ...
      memberdata.set({
        xp: memberdata.get("xp") + calculatedXp,
        totalXP: memberdata.get("totalXP") + calculatedXp
      });
      ...
      const entry: FoxLeveling = new this.guild.client.mongo.leveling({
        guildID: message.guild.id,
        userID: message.author.id,
        level: 1,
        xp: 0,
  ...
  public async rankOf(member: any): Promise<number> {
    const data: FoxLeveling[] = await member.client.mongo.leveling
      .sort("totalXP", "desc")
      .find({ guildID: member.guild.id });
    const mapped: string[] = data.map(c => `${c.get("userID")}`);
  ...
  public static validate(message: FoxMessage): boolean | number {
    ...
    if (mentions.roles.some(r => [message.guild.id].includes(r.id))) {
      return false;
    }
    let substance: number = 0;
    if (text.length > "lol xD".length)
      (substance += 400) * ((substance - 5) / 1995) + 7; // tslint:disable-line
    substance += substantialWords.reduce(
      (num, word) => (text.includes(word) ? num + 2 : num),
      0
    );
    substance -= insubstantialWords.reduce(
