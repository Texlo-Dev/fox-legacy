...
export default class GuildCreate extends Event {
    ...
    public async run(guild: FoxGuild): Promise<void> {
        ...
        await GuildCreate.initializeServer(guild)
            .catch(console.error);
        let channel: any = guild.channels
            .filter(c => c.type === "text" && c.permissionsFor(guild.me).has(["SEND_MESSAGES", "EMBED_LINKS", "VIEW_CHANNEL"])) //tslint:disable-line
        ...
        const embed: MessageEmbed = new MessageEmbed()
            ...
            .addField("Web Dashboard", `To set up ${client.user.username} for your server, please click [here](http://mrfoxbot.xyz) to be directed to our web dashboard, where all packages/commands/permissions and their documentation are listed there.`) // tslint:disable-line
