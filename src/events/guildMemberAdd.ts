...
export default class NewMember extends Event {
    ...
    public async handleLog(member: GuildMember): Promise<void> {
        ...
        const serverlog: TextChannel = member.guild.channels.get(modlog.id) as TextChannel;
        if (!serverlog) { return; }
    ...
    public async run(member: GuildMember): Promise<void> {
        ...
        if (guild.id === "336211307541954560") {
            ...
            const ownerCheck: boolean[] = await this.client.shard.broadcastEval(`this.guilds.some(g => g.ownerID === '${member.id}')`); // tslint:disable-line
    ...
    public async welcomemsgHandle(member: GuildMember): Promise<any> {
        ...
        if (welcomeEnabled && welcomeMessage && welcomeMessage.length && location === "DM") {
            if (!canEmbed) {
                member.send(welcomeMessage.replace(/{user}/g, `${member}`)
                ...
                new MessageEmbed()
                    ...
                    .setDescription(welcomeMessage.replace(/{user}/g, `${member}`).replace(/{server}/g, `${member.guild}`).replace(/{position}/g, `${member.guild.memberCount}`)) // tslint:disable-line
                    .setFooter(this.client.user.username),
                )
                .catch(() => 0);
            }
        } else if (welcomeEnabled && welcomeMessage && location.id) {
            const channel: TextChannel = member.guild.channels.get(location.id) as TextChannel;
            if (channel && !canEmbed) {
                channel.send(welcomeMessage.replace(/{user}/g, `${member}`)
                .replace(/{server}/g, `${member.guild}`)
                .replace(/{position}/g, `${member.guild.memberCount}`));
            } else {
                channel.send(
