...
export default class MUpdate extends Event {
    ...
    public async run(oldMessage: FoxMessage, newMessage: FoxMessage): Promise<void> {
        ...
        if (!enabled) { return; }
        const channelExclude: boolean = oldMessage.guild.config.logExcluded.some(c => c.id === oldMessage.channel.id);
        ...
        if (modlog) {
            const serverlog: TextChannel = oldMessage.guild.channels.get(modlog.id) as TextChannel;
            if (!serverlog) { return; }
