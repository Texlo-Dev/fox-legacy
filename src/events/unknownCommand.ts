...
export default class UnknownCommand extends Event {
  ...
  public async handleCustomCommand(
    message: FoxMessage,
    ...
    return command.execute(message).catch(err =>
      message.error(
        `There was an error when attempting to execute this custom command: ${
          err.message
        }
            `
      )
  ...
  public async run(message: FoxMessage): Promise<any> {
    if (message.guild) {
      ...
      const prefix: RegExpExecArray = new RegExp(
        `^<@!?${this.client.user.id}> |^${UnknownCommand.regExpEsc(
          commandPrefix
        )}`
      ...
      if (message.guild.commands.get(cmdName)) {
        return this.handleCustomCommand(
          message,
          message.guild.commands.get(cmdName),
          args
        );
      }
