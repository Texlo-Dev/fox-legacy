...
export default class extends Event {
  ...
  public async run(message: FoxMessage, level: number): Promise<void> {
    ...
    if (location === "Current Channel") {
      ...
      channel = message.guild.channels.get(location.id);
    }
    ...
    if (msgEnabled && !roles.length) {
      ...
      return message.FoxEmbed(
        ...
        `@${
          message.member.displayName
        }, congratulations on reaching **Level ${level}**! You have been given the following roles: ${roles
          .map(p => `${message.guild.roles.get(p.id)}`)
          .join(", ")}`
      ); // tslint:disable-line
    }
  }
