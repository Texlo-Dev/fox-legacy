...
export default class extends Event {
    ...
    public async run(oState: VoiceState, nState: VoiceState): Promise<void> {
        ...
        const channel: VoiceChannel = oState.channel ? oState.channel : nState.channel;
        ...
        const embed: MessageEmbed = new MessageEmbed()
            .setAuthor(hasJoined ? "Joined Voice Channel" : oState.channel && nState.channel ? "Changed Voice Channel" : "Left Voice Channel", channel.guild.client.user.displayAvatarURL({})) // tslint:disable-line
            .setTimestamp()
