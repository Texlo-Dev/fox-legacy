...
export default class extends Event {
    ...
    public async checkAutoMute(member: GuildMember): Promise<void> {
        ...
        if (guild.config.muteRole && member.roles.has(guild.config.muteRole.id)) {
            const query: ModActions = await this.client.mongo.modactions
                .findOne({ guildID: guild.id, userID: member.id, isMute: true, hasLeft: false, automatic: true });
            query.set({ hasLeft: true });
            await query.save();
        } else {
            const query: ModActions = await this.client.mongo.modactions
                .findOne({ guildID: guild.id, userID: member.id, isMute: true, hasLeft: true, automatic: true });
            if (query) {
    ...
    public async run(member: GuildMember): Promise<void> {
        ...
        const embed: MessageEmbed = new MessageEmbed()
            ...
            .setDescription(`\n**Member Name:** ${member.user.tag}\n**ID:** ${member.id}\n**Joined At:** ${dateFormat(member.joinedAt)}`) // tslint:disable-line
            .setTimestamp()
