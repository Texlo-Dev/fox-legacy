...
export default class FoxCommand extends Command {
  ...
  public static async run(
    ...
    if (!amount) {
      return message.send(
        `The current volume for music is **${
          message.guild.voiceConnection
            ? message.guild.voiceConnection.dispatcher.volume * 10
            : 10
        }0 %**.`
      ); // tslint:disable-line
    }
    if (amount < 1 || amount > 10) {
      return message.error(
        " The supported volume range is 1-10, please try again."
      );
    }
    ...
    if (
      ...
      return message.error(
        "You must be in a voicechannel to change the volume of a current song."
      );
    }
    const serverQueue: Queue = message.guild.queue;
    if (!serverQueue) {
      return message.error(
        " Nothing is playing, so I shouldn't change the volume."
      );
    }
    message.guild.voiceConnection.dispatcher.setVolume(`${volumes[amount]}`);

    return message.send(
      `Successfully set the volume to **${amount}0%** <:check:314349398811475968>`
    );
  }
