...
export default class FoxCommand extends Command {
    ...
    public run(message: FoxMessage): Promise<FoxMessage> {
        ...
        const songTime: string = duration(serverQueue.connection.dispatcher.streamTime, "milliseconds")
            .format("m:ss", { trim: false });
        const songDuration: string = duration(parseFloat(np.length), "seconds")
            .format("m:ss", { trim: false });
        const dispatcherSeconds: number = serverQueue.connection.dispatcher.streamTime / 1000; // tslint:disable-line
        const remaining: string = duration(np.length - dispatcherSeconds, "seconds")
            .format("m:ss", { trim: false });
        const embed: MessageEmbed = new MessageEmbed()
            .setThumbnail(np.thumbnail)
            .setAuthor("Now Playing", message.client.user.displayAvatarURL())
            .setDescription(`**${np.title}**\nAuthor: **${np.author}**\nProgress: **${songTime}/${songDuration} (About ${remaining} left)**`) // tslint:disable-line
