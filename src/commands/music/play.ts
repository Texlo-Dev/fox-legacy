  ...
  public async run(message: FoxMessage, args: string[]): Promise<FoxMessage> {
    ...
    if (!platform || !["yt", "sc"].includes(platform)) {
      return message.error(
        "Invalid platform. Available platforms:\nyt = YouTube\nsc = SoundCloud"
      );
    }
    ...
    if (queue.size > 1 && message.author.upvoter) return message.error("Your song queue limit is currently at 4 songs as a non-upvoter. To increase your queue limit, please upvote the bot on DiscordBots.org, it's free! https://discordbots.org/bot/mrfox");
    else if (queue.size > 4 && message.author.patreonTier < 1)
