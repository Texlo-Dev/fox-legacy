    ...
    return message.send(
      new MessageEmbed()
        ...
        .addField(
          "Status",
          ticket.open
            ? "Open <:checkmark:495362807731060757>"
            : "Closed <:nicexmark:495362785010647041>",
          true
        )
        .addField(
          "Support Agent(s)",
          ticket.agents.map(t => t.tag).join(", ") || "None",
          true
        )
        .addField(
          "Created At",
          moment(ticket.createdAt).format("MMM Do YYYY, h:mm a")
        )
        .setFooter(
