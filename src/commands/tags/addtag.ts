...
export default class FoxCommand extends Command {
    ...
    public async run(message: FoxMessage, args: string[]): Promise<FoxMessage> {
        ...
        const doesExist: Tags = await this.client.mongo.tags.findOne({ guildID: message.guild.id, tagName: tagname });
        if (doesExist) { return message.error("This tag name already exists, please try again."); }
        const tagcontent: string = args.slice(1)
            .join(" ");
        if (!tagcontent) {
            ...
            switch (response) {
                ...
                default:
                    ...
                    entry.save()
                        ...
                        .catch(err => message.error(`There was an error adding that tag.\n\`\`\`${err}\`\`\``));
            }
        }
    }
