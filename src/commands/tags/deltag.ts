...
export default class FoxCommand extends Command {
    ...
    public async run(message: FoxMessage, args: string[]): Promise<FoxMessage> {
        ...
        if (!tagname) {
            const response: string | number = await message.sendPrompt("What tag would you like to delete?", 30000);
            switch (response) {
                case undefined:
                ...
                default:
                    ...
                    if (!tag) { return message.error("Sorry, that tag didn't exist."); }
                    if (
                        tag.get("author") !== message.author.id && !message.guild.perms.check("tag.tageditor", message)
                    ) {
                        return message.error(" Silly you! You can only delete your own tags!");
                    }

                    return tag.remove()
                    .then(() => message.success(`Successfully deleted the tag **${tag.get("tagName")}**.`));
            ...
            if (!tag) { return message.error("Sorry, that tag didn't exist."); }
            if (tag.get("author") !== message.author.id && !message.guild.perms.check("tag.tageditor", message)) {
                return message.error(" Silly you! You can only delete your own tags!");
            }

            return tag.remove()
            .then(() => message.success(`Successfully deleted the tag **${tagname}**.`));
        }
    }
