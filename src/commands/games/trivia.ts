...
export default class FoxCommand extends Command {
  ...
  public async fetchTriviaQ(): Promise<Object> {
    ...
    const answers: string[] = obj.incorrect_answers.map(o =>
      FoxCommand.atob(o)
    );
    answers.splice(
  ...
  public async run(message: FoxMessage): Promise<FoxMessage> {
    const obj: Object = await this.fetchTriviaQ().catch(console.error);
    if (!obj) {
      return message.error(
        "It seems like the trivia API is currently down. Please try again later."
      );
    }
    let trivia: object = game[`${message.guild.id}${message.author.id}`];
    if (!trivia) {
      trivia = game[`${message.guild.id}${message.author.id}`] = {
        correct: []
      };
    }
    const res: boolean | number = await this.sendQuestion(1, message, trivia);
    if (res === 0) {
      return message.send("Successfuly cancelled the game.");
    }
  ...
  public async sendQuestion(
    ...
    let res: any;
    num === 1
      ? (res = await message.sendPrompt(
          `Welcome to trivia! Let's get started!\nCategory: *${
            obj.category
          }*.\n${
            obj.type === "boolean"
              ? `True or False: ${obj.question}\n${obj.available_answers
                  .map(o => `\n**${question++}.** ${o}`)
                  .join("\n")}`
              : `${obj.question}\n${obj.available_answers
                  .map(o => `\n**${question++}.** ${o}`)
    ...
    if (!this.isCorrect(res, obj)) {
      message.channel.send(
        `<:nicexmark:495362785010647041>  Sorry, wrong answer! Correct answer is "${
          obj.correct_answer
        }".`
      ); // tslint:disable-line
    } else {
