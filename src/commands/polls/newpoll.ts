  ...
  public static async run(
    ...
    if (!name) {
      return message.error(
        "Please set a name for this poll that is one word and contains no spaces."
      );
    }
    ...
    const question: string = args.slice(2).join(" ");
    if (!question) {
      return message.send("You must specify a question to ask.");
    }
    if (type === "simple") {
      const m: FoxMessage = await message.send(
        "<a:typing:393848431413559296> Creating a simple poll...."
      );
      message.guild.polls
        .add(name, {
          type,
        ...
        .catch(error =>
          m.edit(
            `<:nicexmark:495362785010647041> I could not start this poll.. ${
              error.message
            }`
          )
        );
