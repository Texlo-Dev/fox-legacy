import { MessageEmbed, GuildMember, TextChannel, Role } from "discord.js";
...
export default class FoxCommand extends Command {
    ...
    public async run(message: FoxMessage, args: string[], prefix: string) {
        ...
        const muteRole: Role = message.guild.config.muteRole
            ...
            : undefined;
        if (!muteRole) {
            return message.error(`Sorry, but there isn't a mute role currently set. Please have the guild owner run \`${await message.guild.config.prefix}config muterole [nameofrole]\` to use this command.`); // tslint:disable-line
        }
        if (!member.roles.has(muteRole.id)) {
            return message.error("This person is not currently muted, ignoring command.")
        ...
        const mem: GuildMember = await member.roles.remove(muteRole)
        .catch(() => undefined);
        if (!mem) { return message.error("I don't have permissions to remove this role. Make sure that the role is below mine, and I have the Manage Role permission."); } // tslint:disable-line
        ...
        if (!modlog) {
            return message.send(`I have unmuted **${mem.user.tag}**, with the reason of **${reason}**. :ok_hand:`);
        }
        const m: any = modlog
