...
export default class FoxCommand extends Command {
    ...
    public async run(message: FoxMessage, args: string[], prefix: string): Promise<FoxMessage> {
        if (!message.guild.me.hasPermission("MANAGE_ROLES")) {
            return message.error("Sorry, I do not have the permission Manage Roles to perform this operation.");
        }
        ...
        const muteRole: Role = message.guild.config.muteRole
            ...
            : undefined;
        if (!muteRole) {
            return message.error(`Sorry, but there isn't a mute role currently set. Please have the guild owner run \`${message.guild.config.prefix}config muterole [nameofrole]\` to use this command.`); // tslint:disable-line
        }
        if (member.roles.has(muteRole.id)) {
            return message.send("This person is already muted, ignoring command.")
            .then(mg => mg.delete({ timeout: 2000 }));
        }
        message.guild.channels.forEach(channel => channel.createOverwrite(muteRole, {
            SEND_MESSAGES: false,
            ADD_REACTIONS: false,
        }));
        ...
        const mem: GuildMember = await member.roles.add(muteRole)
        .catch(() => undefined);
        if (!mem) {
            return sendM.edit("<:nicexmark:495362785010647041> I don't have permissions to assign this role. Make sure that the role is below mine, and I have the Manage Role permission."); // tslint:disable-line
        }
        ...
        const m: Message | Message[] = modlog
            ? await (message.guild.channels.get(modlog.id) as TextChannel).send({ embed }) as Message
            : undefined;
        ...
        if (message.guild.config.msgAfterMod) {
            member.send(`You have been muted on **${message.guild.name}** for ${_duration(time, "milliseconds").format("d [days], h [hours], m [minutes]")}, with the reason of _${reason}_.`) // tslint:disable-line
            .catch(() => 0);
        }
    }
