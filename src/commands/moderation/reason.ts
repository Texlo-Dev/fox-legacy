...
export default class FoxCommand extends Command {
  public static hasPermission(message: FoxMessage): boolean {
    return (
      message.guild.perms.check("mod.banhammer", message) ||
  ...
  public async run(message: FoxMessage, args: string[]): Promise<FoxMessage> {
    ...
    if (!modlog) {
      return message.error("Sorry, no mod-log channel is set up.");
    }
    const channel: TextChannel = message.guild.channels.get(
      modlog.id
    ) as TextChannel;
    if (!channel) {
      return message.error(
        "Sorry, the current mod-log channel does no longer exist."
      );
    }
    ...
    if (!modCase) {
      return message.error("Sorry, that was an invalid case number.");
    }
    if (modCase.get("modID") !== message.author.id) {
      return message.error("This case isn't yours!");
    }
    const newReason: string = args.slice(1).join(" ");
    if (!newReason) {
    ...
    if (!mID) {
      return message.error(
        "It looks like this case does not have a mod-log entry."
      );
    }
    ...
    if (!m) {
      return message.error(
        "It looks like the mod-log entry is no longer in the channel."
      );
    }
    const embed: MessageEmbed = m.embeds[0];
    embed.description = embed.description.replace(
      modCase.get("reasonFor"),
      newReason
    );
    m.edit({ embed });
    ...
    return message.success(
      `Successfully edited the reason for Case#${caseNum}. \`${newReason}\``
    );
  }
}
