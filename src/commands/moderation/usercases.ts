...
export default class FoxCommand extends Command {
  ...
  public async run(message: FoxMessage, args: string[], prefix: string) {
    ...
    const user: FoxUser = (await this.user(
      message.mentions.users.first() || args[1],
      message
    )) as FoxUser;
    if (!user) {
      ...
      if (!query) {
        return message.reply("I couldn't find any recent cases.");
      }
      const mg: FoxMessage = await message.send(
        "<a:typing:393848431413559296> Loading all user cases...."
      );
      const map: ModActions[] = query.filter(
        q => q.get("caseNum") !== undefined
      );
      ...
      const embed: MessageEmbed = new MessageEmbed()
        ...
        .setDescription(stripIndents`
                **All Moderation cases for ${message.guild.name}, Page ${
        paginated.page
      }:**
                ...
                ${(await Promise.all(
                  paginated.items.map(
                    async c =>
                      `**Case#${c.get("caseNum")} -** ${c.get(
                        "action"
                      )}\nMember: ${
                        (await this.client.users.fetch(c.get("userID"))).tag
                      }, Moderator: ${
                        (await this.client.users.fetch(c.get("modID"))).tag
                      }`
                  )
      ...
      if (!query) {
        return message.reply("I couldn't find any recent cases for this user.");
      }
      const mg: FoxMessage = await message.send(
        `<a:typing:393848431413559296> Loading all user cases for ${
          user.tag
        }....`
      );
      const map: ModActions[] = query.filter(
      ...
      const users: string[] = paginated.items.map(async usr => {
        ...
        return `**Case#${user.get("caseNum")} -** ${usr.get(
          "action"
      ...
      const embed: MessageEmbed = new MessageEmbed()
        ...
        .setAuthor("Moderation cases", this.client.user.displayAvatarURL())
        .setDescription(stripIndents`
                **All Moderation cases for ${user.username}, Page ${
        paginated.page
      }:**
                ...
                ${
                  paginated.maxPage > 1
                    ? `Type \`${prefix}usercases [pagenumber]\` to view a specific page.`
                    : ""
                }
                `);
