...
export default class FoxCommand extends Command {
  ...
  public static async run(
    ...
    if (time === "disable" || time === "false") {
      time = 0;
    } else {
      time = FoxClient.spanMs(time) / 1000;
      if (!time) {
        return message.error(
          "You must tell me the time (from 1 second to 2 minutes)"
        );
      }
      if (time < 1 || time > 120) {
        return message.error(
          "Invalid time! Time goes from 1 second to 2 minutes!"
        );
      }
    }
    ...
    message.channel
      ...
      .then(() =>
        message.success(
          `I have ${
            time === 0 ? "disabled" : "set"
          } the slowmode for this channel.`
        )
      )
      .catch(err =>
        message.error(
          `There was an error while performing this operation. ${err}`
        )
      );
  }
