...
export default class FoxCommand extends Command {
  ...
  public async run(message: FoxMessage, args: string[]): Promise<FoxMessage> {
    ...
    const initmsg: FoxMessage = await message.send(
      "<a:typing:393848431413559296> Searching..."
    );
    if (!query) {
      return message.error(" Please specify something to search on Google.");
    }
    try {
      ...
      initmsg.edit(
        new MessageEmbed()
          ...
          .setFooter(this.client.user.username).setDescription(stripIndents`
