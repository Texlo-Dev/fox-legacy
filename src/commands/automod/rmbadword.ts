...
export default class FoxCommand extends Command {
    ...
    public static async run(message: FoxMessage, [...word]: string[]): Promise<void> {
        await message.delete()
            .catch(() => 0);
        const wd: string = word.join(" ");
        if (!wd) { return message.error("Please specify a bad word to remove."); }
        message.guild.config.setArray("badWords", wd)
            .then(() => message.send(`${message.author}: I have removed that word from the blacklist.`))
