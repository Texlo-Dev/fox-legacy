...
export default class FoxCommand extends Command {
    ...
    public async run(message: FoxMessage, args: string[]): Promise<Message | Message[] | void> {
        ...
        if (!scope || scopes.indexOf(scope) < 0) {
            return message.error(`Invalid scope. Available scopes are ${scopes.join(", ")}`);
        }
        ...
        const target: any = scope === "user"
        ? await this.user(args.slice(3)
            .join(" "),   message)
        : scope === "everyone"
        ? { name: "@everyone", id: message.guild.id }
        : await Command.role(args.slice(3)
            .join(" "),      message);
        if (!target) { return message.error("Invalid target specified. Must be valid user or role."); }
        message.guild.perms.add(perm, target, bool ? "allowed" : "denied")
            .then(() => message.FoxEmbed({ header: "Set Permission" }, `Permission \`${perm}\` for ${target.id === message.guild.id ? "Everyone are" : `${target} is`} now set to **${bool}**.`)) // tslint:disable-line
