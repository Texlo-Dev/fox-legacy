...
export default class FoxCommand extends Command {
  ...
  public async run(message: FoxMessage, args: string[]): Promise<FoxMessage> {
    ...
    if (!member) {
      return message.error("No user detected.");
    }
    let roles: string[] = member.roles
      .array()
    ...
    const embed: MessageEmbed = new MessageEmbed()
      ...
      .addField(
        ...
        member.user.presence.activity
          ? member.user.presence.activity.name
          : "Not in-game",
        true
      ) // tslint:disable-line
