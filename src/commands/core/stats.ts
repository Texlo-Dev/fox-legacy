...
export default class FoxCommand extends Command {
    ...
    public async run(message: FoxMessage): Promise<void> {
        ...
        const users: number = (await this.client.shard.fetchClientValues("users.size")).reduce((prev, val) => prev + val, 0);
        const channel: number = (await this.client.shard.fetchClientValues("channels.size")).reduce((prev, val) => prev + val, 0);
        const mem: number = (await this.client.shard.broadcastEval("Math.round(100 * process.memoryUsage().heapUsed / 1024 / 1024 / 1024) / 100")).reduce((prev, val) => prev + val, 0);
        const totalMem: number = Math.round(100 * totalmem() / 1024 / 1024 / 1024) / 100; //tslint:disable-line
