...
const uptime: (time: string) => string = time => duration(time)
...
export default class FoxCommand extends Command {
    ...
    public async run(message: FoxMessage): Promise<void> {
        const shardChannels: number[] = await this.client.shard.fetchClientValues("channels.size");
        const shardGuilds: number[] = await this.client.shard.fetchClientValues("guilds.size");
        const shardUsers: number[] = await this.client.shard.fetchClientValues("users.size");
        const shardMemory: number[] = await this.client.shard.broadcastEval("(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)"); //tslint:disable-line
        const shardUptime: number[] = await this.client.shard.broadcastEval("this.uptime");
