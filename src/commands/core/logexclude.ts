...
export default class FoxCommand extends Command {
  ...
  public async run(message: FoxMessage, args: string[]): Promise<void> {
    ...
    const channel: TextChannel = (await this.channel(
      id,
      message
    )) as TextChannel;
    if (!channel) {
      return message.error("This is not a valid channel.");
    }
    ...
    const embed: MessageEmbed = new MessageEmbed()
      ...
      .setDescription(
        `Successfully excluded server logging in ${channel} <:check:314349398811475968>`
      );
    message.send({ embed });
  }
}
