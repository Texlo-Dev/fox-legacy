  ...
  public async run(
    ...
    if (!args[0] && !message.guild) {
      const embed: MessageEmbed = new MessageEmbed()
        ...
        .setDescription(
          `My current global prefix \`${prefix.replace(
            "f)",
            "f)"
          )}\`. To use commands in a guild channel, just run \`${prefix}command\`.`
        ); // tslint:disable-line
      message.send({ embed });
    } else if (!args[0]) {
      const embed: MessageEmbed = new MessageEmbed()
        ...
        .setFooter(this.client.user.username)
        .setDescription(
          `My current prefix for this server is \`${prefix}\`. To use commands, just run \`${prefix}command\`.`
        ); // tslint:disable-line
      message.send({ embed });
    } else {
      if (!message.guild) {
        return message.reply(
          "You may only change the guild prefix in a guild channel."
        );
      }
      ...
      const embed: MessageEmbed = new MessageEmbed()
        ...
        .setFooter(this.client.user.username)
        .setDescription(
          `Successfully set the new prefix as \`${
            args[0]
