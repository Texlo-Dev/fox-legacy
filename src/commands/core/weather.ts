...
const { Canvas } = cv;
Canvas.registerFont(
  `${process.cwd()}/build/canvas/fonts/Roboto-Regular.ttf`,
  "Roboto"
);
Canvas.registerFont(
  `${process.cwd()}/build/canvas/fonts/RobotoCondensed-Regular.ttf`,
  "Roboto Condensed"
);
Canvas.registerFont(
  `${process.cwd()}/build/canvas/fonts/RobotoMono-Light.ttf`,
  "Roboto Mono"
);
export default class FoxCommand extends Command {
  ...
  public async run(
    ...
    if (!text) {
      return message.error("Missing city.");
    }
    const mg: FoxMessage = await message.send(
      "<a:typing:393848431413559296> Loading Weather....."
    );
    try {
      text = text.replace(/ /g, "");
      const { data } = await axios({
        ...
        params: {
          address: encodeURIComponent(text.replace(/ /g, "+")),
          key: googleAPI
        },
        method: "GET"
      });
      ...
      const params: string = `${data.results[0].geometry.location.lat},${
        data.results[0].geometry.location.lng
      }`;

      const locality: string = data.results[0].address_components.find(loc =>
        loc.types.includes("locality")
      );
      const governing: string = data.results[0].address_components.find(gov =>
        gov.types.includes("administrative_area_level_1")
      ); // tslint:disable-line
      const country: string = data.results[0].address_components.find(cou =>
        cou.types.includes("country")
      );
      const continent: string = data.results[0].address_components.find(con =>
        con.types.includes("continent")
      );

      const city: any = locality || governing || country || continent || {};
      const state: any =
        locality && governing ? governing : locality ? country : {};
      ...
      const chanceofrain: number =
        Math.round((dsky.data.currently.precipProbability * 100) / 5) * 5;
      ...
      const cond: Buffer = await readFile(
        `${process.cwd()}/build/canvas/weather/icons/${theme}/humidity.png`
      );
      const hum: Buffer = await readFile(
        `${process.cwd()}/build/canvas/weather/icons/${theme}/humidity.png`
      );
      const precip: Buffer = await readFile(
        `${process.cwd()}/build/canvas/weather/icons/${theme}/precip.png`
      );
        ...
        .addText(
          city.long_name
            ? (await this.client.translate(
                city.long_name,
                await this.client.locales[message.guild.config.language]
              )).text
            : "Unknown",
          35,
          50
      ...
      message.channel.send({
        files: [{ attachment: img, name: `${geocodelocation}.png` }]
      });
    } catch (error) {
      mg.error(
        `Error while loading weather for this location. ${error.message}`
      );
    }
  }
}
