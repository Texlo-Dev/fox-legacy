...
export default class FoxCommand extends Command {
  ...
  public static async run(
    ...
    if (
      item.maxPurchases &&
      item.buyers.filter(m => m.id === message.author.id).length >=
        item.maxPurchases
    ) {
      return message.error(
    ...
    const resp: string | number = await message.sendPrompt(
      `Are you sure that you want to buy the item **${
        item.name
      }** for **${message.guild.banking.currency +
    ...
    if (!resp) {
      return message.error("Cancelled operation.");
    }
    try {
      await Banking.removeMoney(message.author, {
        guild: message.guild,
        amount: item.price
      });
      ...
      if (item.role) {
        member.roles.add(item.role.id);
      }
      if (item.value) {
        member.send(
          `**Here are the contents of your purchased item:**\n\n${item.value}`
        );
      }
      return message.success(
        `Success! you have bought the **${item.name}** for **${message.guild
          .banking.currency + item.price.toLocaleString()}**.`
      ); // tslint:disable-line
    } catch (error) {
      return message.error(
        `Oops, there was an error during this process. ${error}`
      );
    }
  }
