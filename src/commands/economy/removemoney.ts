  ...
  public async run(message: FoxMessage, args: string[]): Promise<FoxMessage> {
    ...
    if (!amount) {
      return message.error(
        " Please specify an amount of :yen: to remove from this user."
      );
    }
    ...
    if (result === "Account") {
      return message.error("Sorry, this user doesn't have a bank account.");
    }
    if (result === "Failed") {
      return message.error(
    ...
    if (result === true) {
      return message.success(
        `Successfully removed ${message.guild.banking.currency +
          amount.toLocaleString()} from **${user.tag}**.`
      ); // tslint:disable-line
    }
  }
}
