    ...
    async run(message, args) {
        ...
        if (!m) {
            const embed = new MessageEmbed()
                ...
                .setDescription(`Current level up message: ${await message.guild.config.levelMsg ? `"${await message.guild.config.levelMsg}"` : 'None'}`);
            message.send({ embed });
        } else {
            ...
            const embed = new MessageEmbed()
                ...
                .setTimestamp()
                .setFooter(this.client.user.username)
                .setDescription(` <:check:314349398811475968> Successfully set Level Up message as "${m}"`);
            message.send({ embed });
        }
    }
