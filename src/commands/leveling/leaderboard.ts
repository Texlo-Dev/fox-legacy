...
export default class FoxCommand extends Command {
  ...
  public async run(
    ...
    const mapped: FoxLeveling[] = data
      .filter(d => d.get("level"))
      .map(c => c.get());
    ...
    const num: string[] = paginated.items.map(async mem => {
      const check: FoxUser = await this.client.users
        .fetch(mem.userID)
        .catch(() => undefined);

      return `**${++rank}.** Level ${mem.level} (${parseInt(
        mem.xp
      ).toLocaleString()}) - ${check ? check.tag : "Deleted user"}\n`; // tslint:disable-line
    });
    const str: string = `${(await Promise.all(num)).join(" ")}\n${
      paginated.maxPage > 1
        ? `To see a specific page, just run ${prefix}leaderboard [pagenumber].`
        : ""
    }`; // tslint:disable-line
