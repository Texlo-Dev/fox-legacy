...
export default class FoxCommand extends Command {
    ...
    public static async run(message: FoxMessage, [...name]: string[]): Promise<FoxMessage> {
        ...
        const m: FoxMessage = await message.send("<a:typing:393848431413559296> Pausing giveaway....");
        message.guild.giveaways.get(name.join(" "))
            .pause()
            .then(() => m.edit("<:checkmark:495362807731060757> Successfully paused the giveaway."))
            .catch(error =>
                m.edit(`<:nicexmark:495362785010647041> I could not pause this giveaway. ${error.message}`));
    }
