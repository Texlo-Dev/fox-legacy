...
export default class FoxCommand extends Command {
  ...
  public static async run(
    ...
    if (!name.join(" ")) {
      return message.error(" Please specify a new giveaway to end.");
    }
    ...
    const m: FoxMessage = await message.send(
      "<a:typing:393848431413559296> Ending giveaway...."
    );
    message.guild.giveaways
      ...
      .then(() =>
        m.edit(
          "<:checkmark:495362807731060757> Successfully ended the giveaway."
        )
      )
      .catch(error =>
        m.edit(
          `<:nicexmark:495362785010647041> I could not end this giveaway. ${
            error.message
          }`
        )
      );
