...
export default class FoxCommand extends Command {
    ...
    async run(message) {
        try {
            ...
            const m = await message.send('a:typing:393848431413559296> Starting giveaway....');
            message.guild.giveaways.set(this.args.prize, {
                ...
                m.edit(`<:nicexmark:495362785010647041> Could not add this giveaway. ${error.message}`);
            });
        } catch (error) {
    ...
    async getTime(message) {
        ...
        if (res && this.client.spanMs(res) >= 25000 && this.client.spanMs(res) <= 3888000000) {
            ...
            message.error('Invalid Time Format/option. Giveaway must be at least 25 seconds, and no longer than 45 days.');
    ...
    async getPrize(message) {
        const res = await message.sendPrompt(`What would you like to name the prize?`, 30000).catch(() => null);
        if (res) {
            this.args.prize = res;
            return this.args;
        } else if (res === 0) {
    ...
    async getChannel(message) {
        const res = await message.sendPrompt(`What channel would you like the giveaway to be held in?`, 30000).catch(() => null);
    ...
    async getMaxWinners(message) {
        const res = await message.sendPrompt(`How many winners would you like to allow?`, 30000).catch(() => null);
    ...
    async confirm(message) {
        const res = await message.sendPrompt(`Check this info! Is it correct? Respond with Yes to start the giveaway.\nTime: ${this.args.time}\nPrize: ${this.args.prize}\nChannel: ${this.args.channel}\nMax Winners: ${this.args.maxWinners}`, 30000).catch(() => null);
